...
def compute_wayside_outputs(block_state, block_lengths, switches_def,
    ...
    # Set when train enters C/B/A (blocks 1-12).
    # Cleared only when train moves PAST block 13 (block 14 or higher in D
    # section). We must keep the flag set while block 13 is occupied so that
    # block 13's junction switch_state stays in the (1,13) position long
    # enough for the train to transition cleanly off block 13 to block 14.
    # If we cleared it as soon as block 13 became occupied, SW1 would flip
    # to normal mid-transition, block 13's chosen_prev would flip to 12, and
    # the train at 13 would head back to 12 instead of forward to 14.
    _sw12 = switches_def.get("SW12")
    if _sw12:
        _on_abc = any(b in occupied for b in range(1, 13))
        if _on_abc:
            state["a_loop_active"] = True
        # Clear only when train has moved BEYOND block 13 (into 14+ of D section)
        if any(b in occupied for b in range(14, 17)):
            state["a_loop_active"] = False
    _a_loop_active = bool(state.get("a_loop_active", False))

    # ── F-loop reverse pass flag (SW28 routing memory) ────────────────────────
    # SW28 normal=29 (forward F→G), reverse=150 (Z→F return).
    # When the train returns from Z, the route is 150→28→27→…→21.
    # Block 150 is in WG2, so this WG1 wayside cannot see it directly via
    # block_state. We pull cross-wayside occupancy hints from state["_external_occ"]
    # which the dashboard injects from track-model push.
    #
    # Block 28 has an explicit next override (e29,27), so with switch_state=1
    # (SW28 reverse), chosen_next=27 — letting the reverse pass continue
    # 150→28→27→...→21 correctly. The flag is set when block 150 is occupied
    # (entry pending) and stays True while the train is in F (any of 21-28).
    # It clears once the train exits F (no F block occupied) AND block 150
    # is also empty.
    #
    # During the F-FORWARD pass, the train enters F from block 20→21 with
    # block 150 already empty for many ticks — so the flag is False before
    # the train enters F, and stays False through 21..27..28→29.
    _sw28 = switches_def.get("SW28")
    if _sw28:
        ext_occ = state.get("_external_occ", {}) or {}
        block_150_occ = bool(ext_occ.get(150, False))
        _f_any_occ = any(b in occupied for b in range(21, 29))   # any F block 21..28

        # Set flag when train is at 150 (about to enter F via SW28 reverse).
        if block_150_occ:
            state["f_loop_reverse"] = True
        # Clear when train fully exits F AND is not at 150.
        if not _f_any_occ and not block_150_occ:
            state["f_loop_reverse"] = False
    _f_loop_reverse = bool(state.get("f_loop_reverse", False))
    ...
    for sw_id, sw in switches_def.items():
        ...
        # ── RULE 0: Loop routing memory (must run BEFORE the host-occupied
        # safety rule, because RULE 1 forces "normal" which is wrong when the
        # train is transiting a switch host on a reverse path) ───────────────
        # SW77: forward exit through 77→101 during 2nd N pass.
        # SW85: reverse entry from Q (100→85) and reverse exit (85→84) during 2nd N pass.
        # SW28: reverse entry from Z (150→28) during F-loop return pass.
        # SW57: reverse when train at 57 (yard return) or 0 reachable.
        if sw_id == "SW77":
        ...
        if sw_id == "SW85":
            ...
            continue
        if sw_id == "SW28":
            switch_states[sw_id] = "reverse" if _f_loop_reverse else "normal"
            continue
        if sw_id == "SW1":
            switch_states[sw_id] = "reverse" if _a_loop_active else "normal"
            continue
        if sw_id == "SW57":
            # Block 57 is the last station block (Overbrook) before yard.
            # When the train is on 57, the next route step is always block 0
            # (yard), so SW57 must be reverse. Also reverse if yard (0) is
            # within authority reach (so SW57 flips a few blocks before 57
            # and stays reverse through the transition).
            switch_states[sw_id] = "reverse" if (host in occupied or 0 in all_reach) else "normal"
            continue
        ...
        # ── RULE 2: Yard exit switch (SW62) ───────────────────────────────
        if sw_id == "SW62":
            switch_states[sw_id] = "normal"
            continue
        # SW57 is now handled in RULE 0 above (so it survives RULE 1's
        # host-occupied-forces-normal short-circuit when the train arrives
        # on block 57 itself).

        # ── RULE 3: General merge rule ────────────────────────────────────
...
class WaysideFrame(tk.Frame):
    ...
    def receive_live_data(self, line_name, block_data):
        ...
        # Cross-wayside occupancy hints. Each wayside owns a contiguous
        # block range, but some switch logic depends on knowing whether
        # blocks in OTHER waysides are occupied. We track per-line occupancy
        # for ALL blocks here; the compute call later passes the relevant
        # hints into the wayside's state dict.
        if not hasattr(self, "_external_occ"):
            self._external_occ = {}
        line_occ = self._external_occ.setdefault(line_name, {})
        for blk, data in block_data.items():
            blk_n = _norm_blk(blk)
            line_occ[blk_n] = bool(data.get("occupied", False))
    ...
    def _refresh(self, *_):
        ...
        for wid, ws in self.waysides.items():
            ...
            # Inject cross-wayside occupancy hints. Some switch logic
            # (e.g. SW28 reverse entry from block 150 in WG2) needs to
            # know occupancy of blocks owned by a different wayside.
            ws_line = ws.get("line", "Green")
            ext_occ = getattr(self, "_external_occ", {}).get(ws_line, {})
            ws_state["_external_occ"] = dict(ext_occ)
            try:
