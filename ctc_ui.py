...
# -----------------------------
# =============================================================================
# GREEN LINE AUTOMATIC SCHEDULE
# =============================================================================
# Full explicit block sequence for one complete Green Line trip.
# Derived from greenline.csv directionality and switch connectivity.
#
# Route:
#   Yard(0) -SW62-> K(63-68) -> L(69-73) -> M(74-76) ->
#   N+(77->85) -> O+(86-88) -> P+Q+(89-100) -SW100-> N-(85->77) -SW77->
#   R(101) -> S(102-104) -> T(105-109) -> U(110-116) -> V(117-121) ->
#   W(122-143) -> X(144-146) -> Y(147-149) -> Z(150) -SW150->
#   F-(28->21) -> E-(20->17) -> D-(16->13) -> C-(12->7) -> B-(6->4) ->
#   A-(3->1) -SW1-> D+(13->16) -> E+(17->20) -> F+(21->28) -SW28->
#   G(29-32) -> H(33-35) -> I(36->57) -SW57-> Yard(0)
#
# Switch summary:
#   SW62 (blk 62): Yard(0) -> 63  (yard exit, J skipped)
#   SW100(blk100): 100 -> 85      (N-loop entry, reverse merge)
#   SW77 (blk 77): 77 -> 101      (N-loop exit to R)
#   SW150(blk150): 150 -> 28      (Z to F)
#   SW1  (blk  1): 1  -> 13      (A-loop back to D)
#   SW28 (blk 28): 28 -> 29      (F to G)
#   SW57 (blk 57): 57 -> 0       (yard entry)

_GREEN_OUTBOUND_PATH = (
    [0] +
    list(range(63, 69))      +  # K  63-68
    list(range(69, 74))      +  # L  69-73
    list(range(74, 77))      +  # M  74-76
    list(range(77, 86))      +  # N  first pass  77->85 (bidir +)
    list(range(86, 89))      +  # O  86-88       (bidir +)
    list(range(89, 101))     +  # P+Q 89-100     (uni +)
    list(range(85, 76, -1))  +  # N  second pass 85->77 (bidir -, SW100->85)
    list(range(101, 122))    +  # R(101)+S+T+U+V  (SW77: 77->101)
    list(range(122, 151))    +  # W+X+Y+Z(150)   (uni +)
    list(range(28, 20, -1))  +  # F  28->21       (bidir -, SW150: 150->28)
    list(range(20, 16, -1))  +  # E  20->17       (bidir -)
    list(range(16, 12, -1))  +  # D  16->13       (bidir -)
    list(range(12, 6, -1))   +  # C  12->7        (bidir -)
    list(range(6,  3, -1))   +  # B  6->4         (uni -)
    list(range(3,  0, -1))   +  # A  3->1         (uni -)
    list(range(13, 17))      +  # D  13->16       (bidir +, SW1: 1->13)
    list(range(17, 21))      +  # E  17->20       (bidir +)
    list(range(21, 29))      +  # F  21->28       (bidir +)
    list(range(29, 36))      +  # G(29-32)+H(33-35) (SW28: 28->29)
    list(range(36, 58))      +  # I  36->57       (uni +)
    [0]                         # Yard            (SW57: 57->0)
)

_GREEN_RETURN_PATH = _GREEN_OUTBOUND_PATH

_GREEN_ROUTE_LENGTHS = {
    0: 100,
    **{b: 100 for b in range(63, 69)},
    **{b: 100 for b in range(69, 74)},
    **{b: 100 for b in range(74, 77)},
    **{b: 300 for b in range(77, 86)},
    86:100, 87:87, 88:100,
    **{b: 75  for b in range(89, 98)},
    98:75, 99:75, 100:75,
    101:35,
    102:100, 103:100, 104:80,
    105:100, 106:100, 107:90, 108:90, 109:90,
    **{b: 100 for b in range(110, 117)},
    117:50, 118:50, 119:40, 120:40, 121:40,
    **{b: 50  for b in range(122, 144)},
    144:50, 145:50, 146:50,
    147:50, 148:184, 149:40,
    150:35,
    **{b: 300 for b in range(21, 29)},
    **{b: 150 for b in range(17, 21)},
    **{b: 150 for b in range(13, 17)},
    **{b: 100 for b in range(7,  13)},
    **{b: 100 for b in range(4,  7)},
    **{b: 100 for b in range(1,  4)},
    **{b: 50  for b in range(29, 36)},
    **{b: 50  for b in range(36, 58)},
}

_GREEN_ROUTE_SPEEDS = {
    0: 30,
    **{b: 70 for b in range(63, 69)},
    **{b: 40 for b in range(69, 74)},
    **{b: 40 for b in range(74, 77)},
    **{b: 70 for b in range(77, 86)},
    86:25, 87:25, 88:25,
    **{b: 25 for b in range(89, 101)},
    101:26,
    102:28, 103:28, 104:28,
    **{b: 28 for b in range(105, 110)},
    **{b: 30 for b in range(110, 117)},
    **{b: 15 for b in range(117, 122)},
    **{b: 20 for b in range(122, 151)},
    **{b: 70 for b in range(21, 29)},
    **{b: 60 for b in range(17, 21)},
    **{b: 70 for b in range(13, 17)},
    **{b: 45 for b in range(7,  13)},
    **{b: 45 for b in range(4,  7)},
    **{b: 45 for b in range(1,  4)},
    **{b: 30 for b in range(29, 36)},
    **{b: 30 for b in range(36, 58)},
}


def _build_route_times(path, lengths, speeds):
    times = [0.0]
    for blk in path[1:]:
        l = lengths.get(blk, 50.0)
        s = max(1.0, speeds.get(blk, 30.0)) * 1000.0 / 3600.0
        times.append(times[-1] + l / s)
    return times


_GREEN_ROUTE_TIMES = _build_route_times(
    _GREEN_OUTBOUND_PATH, _GREEN_ROUTE_LENGTHS, _GREEN_ROUTE_SPEEDS
)
_GREEN_ROUTE_TOTAL_SEC = _GREEN_ROUTE_TIMES[-1]  # ~1740 s (~29 min)

_G_MIN = 60
DEFAULT_GREEN_WAYPOINTS = [
    (0,                           "Yard", 0,   "Yard"),
    (int(2.6  * _G_MIN),          "N",   77,   "N-entry"),
    (int(7.8  * _G_MIN),          "N",   85,   "N-loop"),
    (int(8.1  * _G_MIN),          "R",   101,  "R"),
    (int(11.1 * _G_MIN),          "W",   122,  "W-entry"),
    (int(14.4 * _G_MIN),          "Z",   150,  "Z"),
    (int(20.4 * _G_MIN),          "A",   1,    "A"),
    (int(21.3 * _G_MIN),          "D",   13,   "D-return"),
    (int(24.3 * _G_MIN),          "G",   29,   "G-entry"),
    (int(25.0 * _G_MIN),          "I",   36,   "I-entry"),
    (int(_GREEN_ROUTE_TOTAL_SEC), "Yard", 0,   "Yard"),
]

_GREEN_NUM_TRAINS    = 10
_GREEN_TRAIN_GAP_SEC = 360  # 6 minutes between trains
...
GREEN_STATION_TO_START["Yard"] = ("Yard", 0)  # Yard = block 0 (no track-diagram visual)
...
GREEN_LINE_BLOCKS = {
    ...
    "I": [(36, 50, 0.0, 30), (37, 50, 0.0, 30), (38, 50, 0.0, 30),
          ...
          (57, 50, 0.0, 30)],
    "Yard": [(0, 100, 0.0, 30)],   # block 0 = yard/depot (no visual on track model)
...
def _yard_dispatch_start_block(line_short: str) -> int | None:
    ...
    Green line trains sit in the yard at block 0.
    SW62 (host=62) opens the exit to block 63 when they depart.
...
def _interpolated_block_between_waypoints(
    ...
    """
    Interpolate block position from start_block to end_block.

    For the Green Line return segment (141 → 0) the explicit
    _GREEN_RETURN_PATH is used so the train follows 141→150→28→...→0
    instead of the generic forward wrap 141→142→...→150→1→2→...
    """
    f = max(0.0, min(1.0, float(fraction)))
    if start_block == end_block:
        return int(start_block)

    # ── Green Line return path: 141 → 0 via _GREEN_RETURN_PATH ───────────────
    # Triggered whenever start_block is on the return path and end_block is 0.
    if end_block == 0 and start_block in _GREEN_RETURN_PATH:
        try:
            idx_start = _GREEN_RETURN_PATH.index(start_block)
            total_steps = len(_GREEN_RETURN_PATH) - 1 - idx_start
            if total_steps > 0:
                step = int(round(total_steps * f))
                return _GREEN_RETURN_PATH[min(idx_start + step,
                                              len(_GREEN_RETURN_PATH) - 1)]
        except ValueError:
            pass

    # ── Outbound: block 0 → 63 (skip 58-62) ──────────────────────────────────
    if start_block == 0:
        # Jump straight to 63 on first movement tick; interpolate 63→end normally.
        if end_block >= 63:
            total_steps = end_block - 63
            steps_now   = int(round(total_steps * f))
            return 63 + steps_now
        return 63

    # ── General forward (or wrap-around) interpolation ────────────────────────
    if end_block >= start_block:
...
class TrackDiagramWidget(QWidget):
    ...
    def paintEvent(self, _evt):
        ...
        # Track how many stations have already been drawn at each anchor so
        # multiple stations sharing a section (e.g. Overbrook II, Inglewood II,
        # Central II all in W) get vertically offset and don't overlap.
        _anchor_count: dict[str, int] = {}

        def _draw_station_lights(station_name: str, anchor_section: str):
            anchor = label_pts.get(anchor_section)
            if anchor is None:
                return
            # Offset additional stations on the same anchor by 16px each.
            idx = _anchor_count.get(anchor_section, 0)
            _anchor_count[anchor_section] = idx + 1
            y_offset = -34 - (idx * 16)
            base = anchor + QPoint(0, y_offset)
            # Only one signal per station now — draw a single light AT the
            # station block. Goes red when the station block is occupied.
            col = self._station_light_color(station_name, "at")
            fill, ring = _LIGHT_COLORS[col]
            p.setPen(QPen(ring, 2))
            p.setBrush(fill)
            p.drawEllipse(base, 6, 6)
...
class RedLineDiagramWidget(TrackDiagramWidget):
    ...
    def paintEvent(self, _evt):
        ...
        # ── Station lights (single light per station) ────────────────────────
        _LIGHT_COLORS = {
            "green":  (QColor("#22aa44"), QColor("#115522")),
            "yellow": (QColor("#ddaa00"), QColor("#886600")),
            "red":    (QColor("#cc2222"), QColor("#881111")),
        ...
        def _draw_station_lights(station_name: str, anchor_section: str):
            ...
            # Only one signal per station — draw a single light AT the
            # station block. Goes red when the station block is occupied.
            col = self._station_light_color(station_name, "at")
            fill, ring = _LIGHT_COLORS[col]
            p.setPen(QPen(ring, 2))
...
class MainWindow(QMainWindow):
    ...
    def _on_load_schedule(self):
        ...
        if schedule == "Default Green":
            ...
            self._auto_dispatched = set()   # reset so trains re-dispatch on reload
    ...
    def _poll_active_trains(self):
        ...
        if getattr(self, "_sim_running", False) and not getattr(
            ...
            # Timer fires every 100 ms (0.1 s real time).
            # At slider=10 → 1x real time: add 0.1 sim-sec per tick.
            # At slider=100 → 10x real time: add 1.0 sim-sec per tick.
            self._sim_time_sec += self.speed_slider.value() * 0.01
        ...
        # ── Auto schedule: dispatch real trains at their scheduled times ─────
        # Instead of computing phantom positions, write "Auto-N" entries to
        # _external_trains so launch_system.py spawns real track_model.Train
        # objects. Position comes from wayside occupancy below.
        _hide_sched = getattr(self, "_integrated_hide_schedule_trains", False)
        if (
            getattr(self, "_sim_schedule", None) == "Default Green"
            and not _hide_sched
            and getattr(self, "_integrated_sim_clock_from_launcher", False)
        ):
            if not hasattr(self, "_auto_dispatched"):
                self._auto_dispatched = set()
            end_sec = _GREEN_ROUTE_TOTAL_SEC
            for i in range(_GREEN_NUM_TRAINS):
                train_num   = i + 1
                train_start = i * _GREEN_TRAIN_GAP_SEC
                train_t     = self._sim_time_sec - train_start
                auto_key    = f"Auto-{train_num}"

                # Dispatch when sim time reaches this train's start time
                if (train_t >= 0
                        and auto_key not in self._auto_dispatched
                        and auto_key not in getattr(self, "_external_trains", {})):
                    if not hasattr(self, "_external_trains"):
                        self._external_trains = {}
                    # Destination = block 57 (last I-section block, just before
                    # SW57 routes to yard). When the train arrives at 57, the
                    # reroute logic sets dest to yard(0) and SW57 opens.
                    self._external_trains[auto_key] = {
                        "line":       "Green Line",
                        "section":    "Yard",
                        "block":      0,
                        "origin":     "Yard",
                    ...
                    self._auto_dispatched.add(auto_key)

        # ── Build train list from wayside occupancy ────────────────────────────
        # Positions come from what the wayside reports as occupied — the real
        # system architecture where track model → wayside → CTC for occupancy.
        trains = []
        wayside_occ = {}
        if self._shared is not None:
            try:
                ws_out = self._shared.get_wayside_outputs("Green")
                wayside_occ = ws_out.get("occupied", {}) if ws_out else {}
            except Exception:
                wayside_occ = {}

        # Build a block→train info lookup from _external_trains so we can
        # show destination for trains identified only by block occupancy.
        _block_to_train: dict[int, dict] = {}
        for tid, info in getattr(self, "_external_trains", {}).items():
            try:
                bn = int(info.get("block", -1))
            except (TypeError, ValueError):
                bn = -1
            if bn > 0:
                _block_to_train[bn] = {**info, "_tid": tid}

        # One table row per occupied block (positions from wayside)
        for bn, occ in wayside_occ.items():
            if not occ:
                continue
            try:
                bn = int(bn)
                ...
                continue
            section = _GREEN_BLOCK_INFO.get(bn, ("?", 0.0, 0.0))[0]
            station = GREEN_BLOCK_STATIONS.get(bn, "")
            detail  = f"Blk {bn}"
            if station:
                detail += f" — {station}"

            # Look up destination from _external_trains if we have a match
            tinfo    = _block_to_train.get(bn, {})
            tid      = tinfo.get("_tid", f"Blk {bn}")
            dest_nm  = (tinfo.get("dest") or "").strip()
            dest_blk = tinfo.get("dest_block")
            if dest_blk is not None and bn == int(dest_blk):
                dest_disp = f"✓ At {dest_nm}" if dest_nm else "✓ At destination"
            elif dest_nm:
                dest_disp = f"→ {dest_nm}"
            else:
                dest_disp = "—"
        ...
        # Fallback: include any dispatched trains not yet visible in wayside
        # (just dispatched this tick, or wayside hasn't polled yet).
        # Includes Train-*, Manual-*, Auto-* entries.
        for tid, info in getattr(self, "_external_trains", {}).items():
            try:
                bn = int(info.get("block", -1))
            except (TypeError, ValueError):
            ...
            if bn <= 0:
                continue
            # Skip if already shown via wayside occupancy
            if wayside_occ.get(bn):
                continue
            line    = info.get("line", "").replace(" Line", "")
            section = info.get("section", "?")
    ...
    def _auto_route_arrived_trains_to_yard(self):
        """
        When an external train reaches its destination, automatically
        retarget it to Yard (block 0) so SW57 routes it in.
        Works for both CTC phantom trains and physical track-model trains.
        """
        for tid, info in getattr(self, "_external_trains", {}).items():
            ...
            if dest_block is None:
                continue
            try:
                cur_block      = int(info.get("block", -1))
            ...
            if dest_name.lower() == "yard" or dest_block_int == 0:
                continue
...
def _mw_advance_external_trains(self) -> None:
    """
    Advance manually-dispatched trains one simulation step forward along the
    track.  For Green Line trains the explicit _GREEN_OUTBOUND_PATH is used
    so the N-loop, Z-wrap, and yard entry/exit all work correctly.

    Called every 100 ms tick from _poll_active_trains.
    Skipped entirely when launch_system is managing positions.
    """
    ...
    # sim_seconds_per_tick: at slider=10 → 0.1 sim-sec per 100ms tick (1x real)
    sim_seconds_per_tick = slider * 0.01
    ...
    for tid, info in list(getattr(self, "_external_trains", {}).items()):
        ...
        # ── Green Line: follow _GREEN_OUTBOUND_PATH by index ──────────────────
        if "Green" in line_full:
            path    = _GREEN_OUTBOUND_PATH
            lengths = _GREEN_ROUTE_LENGTHS
            speeds  = _GREEN_ROUTE_SPEEDS

            # Initialise path index if not set
            if "_path_idx" not in info:
                try:
                    # Find first occurrence of cur_block in path
                    info["_path_idx"] = path.index(cur_block)
                except ValueError:
                    info["_path_idx"] = 0
                info["dist_in_block_m"] = 0.0

            idx = int(info["_path_idx"])
            idx = max(0, min(idx, len(path) - 1))

            # Arrived at end of path (yard)?
            if idx >= len(path) - 1:
                info["block"]   = path[-1]
                info["section"] = _GREEN_BLOCK_INFO.get(path[-1], ("Yard",50,30))[0]
                info["dist_in_block_m"] = 0.0
                continue

            cur_blk     = path[idx]
            length_m    = lengths.get(cur_blk, 50.0)
            speed_kmh   = max(1.0, speeds.get(cur_blk, 30.0))
            delta_m     = speed_kmh * (1000.0 / 3600.0) * sim_seconds_per_tick
            dist        = info.get("dist_in_block_m", 0.0) + delta_m

            while dist >= length_m and idx < len(path) - 1:
                dist    -= length_m
                idx     += 1
                cur_blk  = path[idx]
                length_m = lengths.get(cur_blk, 50.0)
            ...
            sec = _GREEN_BLOCK_INFO.get(cur_blk, ("?", 50, 30))[0]
            info["block"]           = cur_blk
            info["section"]         = sec
            info["dist_in_block_m"] = dist
            info["_path_idx"]       = idx
            info["suggested_speed_kmh"] = speeds.get(cur_blk, 30.0)
            continue
        # ── End Green Line path ───────────────────────────────────────────────

        # Red Line: simple sequential block+1 movement (unchanged)
        ...
        sec = block_info.get(cur_block, (info.get("section","A"), 50, 30))[0]
    ...
    for tid, info in list(getattr(self, "_external_trains", {}).items()):
        ...
        # ── Return-to-yard path (Green line only) ─────────────────────────────
        # When the train is flagged as returning, step along _GREEN_RETURN_PATH
        # instead of the normal cur+1 sequential logic.
        if info.get("_returning_to_yard") and "Green" in line_full:
            path = _GREEN_RETURN_PATH
            idx  = int(info.get("_return_path_idx", 0))
            # Clamp index to valid range.
            idx = max(0, min(idx, len(path) - 1))

            # Already at the last block in path (yard)?
            if idx >= len(path) - 1:
                info["block"]           = path[-1]
                info["section"]         = block_info.get(path[-1], ("J", 50, 30))[0]
                info["dist_in_block_m"] = 0.0
                info["_returning_to_yard"] = False
                continue

            cur_blk   = path[idx]
            sec, length_m, speed_kmh = block_info.get(cur_blk, ("J", 50.0, 30.0))
            if speed_kmh <= 0:
                speed_kmh = 30.0
            ...
            # Arrived at yard (last block)?
            if idx >= len(path) - 1:
                cur_blk = path[-1]
                dist    = 0.0
                sec     = block_info.get(cur_blk, ("J", 50.0, 30.0))[0]
                info["_returning_to_yard"] = False

            info["block"]            = cur_blk
            info["section"]          = sec
            info["dist_in_block_m"]  = dist
            info["_return_path_idx"] = idx
            continue
        # ── End return path ───────────────────────────────────────────────────

        # ── Outbound yard exit: block 0 → 63 (SW62 opens, skip 58-62) ────────
        if cur_block == 0 and "Green" in line_full:
            cur_block = 63
            sec, _, _ = block_info.get(63, ("K", 100.0, 70.0))
            info["block"]           = cur_block
            info["section"]         = sec
            info["dist_in_block_m"] = 0.0
            continue
...
def _mw_build_ctc_block_state(self, trains: list) -> None:
        ...
        try:
            if sk is not None:
                ...
                if db is not None:
                    dest_bn = int(db)
                    if ls == "Green":
                        # Use path index to populate only blocks between cur and dest
                        try:
                            path = _GREEN_OUTBOUND_PATH
                            path_idx = int(tinfo.get("_path_idx", -1))
                            if path_idx < 0:
                                path_idx = next(
                                    (i for i in range(len(path)) if path[i] == cur_bn), 0
                                )
                            # Find dest_bn at or after path_idx
                            dest_idx = next(
                                (i for i in range(path_idx, len(path)) if path[i] == dest_bn),
                                None
                            )
                            if dest_idx is not None:
                                for rb in path[path_idx:dest_idx + 1]:
    ...
    def _authority_km(line_short: str, cur_bn: int) -> float:
        """
        Sum block lengths from cur_bn to dest along the actual travel path.
        For Green Line uses _GREEN_OUTBOUND_PATH (covers full route including
        N-loop, Z-wrap, and return to yard). Falls back to single block length
        ...
        if dest == cur_bn:
            return 0.0
        return _authority_to_dest_km(line_short, cur_bn, dest)

    def _authority_to_dest_km(line_short: str, cur_bn: int, dest_bn: int) -> float:
        """Authority from cur_bn to dest_bn along the travel path, in km."""
        block_info = _GREEN_BLOCK_INFO if line_short == "Green" else _RED_BLOCK_INFO
        if dest_bn == cur_bn:
            return 0.0
        # Green Line: use _GREEN_OUTBOUND_PATH for path-aware distance.
        # Find the FIRST occurrence of cur_bn at or after the last occurrence
        # of dest_bn to handle repeated blocks (N-loop, D/E/F second pass).
        if line_short == "Green":
            path = _GREEN_OUTBOUND_PATH
            lengths = _GREEN_ROUTE_LENGTHS
            # Find all occurrences of cur_bn
            for idx_cur in range(len(path)):
                if path[idx_cur] != cur_bn:
                    continue
                # Find next occurrence of dest_bn at or after idx_cur
                for idx_dest in range(idx_cur, len(path)):
                    if path[idx_dest] == dest_bn:
                        total_m = sum(
                            lengths.get(b, 50.0)
                            for b in path[idx_cur:idx_dest + 1]
                        )
                        return total_m / 1000.0
            # Not found on path — fall through to simple sum
        # Red Line / fallback: simple sequential distance
        if dest_bn != cur_bn:
            max_b = max(block_info.keys()) if block_info else cur_bn
            total_m = sum(
                block_info.get(b, ("", 50.0, 0.0))[1]
                for b in range(min(cur_bn, dest_bn), max(cur_bn, dest_bn) + 1)
            )
            return total_m / 1000.0
        return 0.0
    ...
    for t in trains:
        ...
        # block may be a raw int (from launch_system) or "Blk N" string (from CTC table)
        if isinstance(blk_raw, (int, float)):
            bn = int(blk_raw)
        else:
            import re as _re
            m = _re.search(r"\bBlk\s+(\d+)\b", str(blk_raw))
            if not m:
                continue
            bn = int(m.group(1))
        ...
        # Block 0 = yard. Push authority onto block 63 (first real block after
        # SW62 yard exit) so the wayside computes green on that block and the
        # train controller gets authority to move out of the yard.
        if bn == 0 and line_short == "Green":
            dest = next(
                (int(tinfo.get("dest_block", 0))
                 for tinfo in ext.values()
                 if int(tinfo.get("block", -1)) == 0),
                0
            )
            auth_km = _authority_to_dest_km("Green", 63, dest) if dest else 5.0
            _, _, spd63 = _GREEN_BLOCK_INFO.get(63, ("K", 100.0, 70.0))
            target[63] = {
                "cmd_speed": spd63,
                "authority": auth_km,
            }
        ...
        # CTC only pushes speed/authority — occupancy comes from track model
        target[bn] = {
    ...
    # ── Return-route switch lookahead ─────────────────────────────────────────
    # For Green Line return trains we need two switches to fire correctly:
    #
    # ── Switch lookahead for Green Line route ────────────────────────────────
    # Several switches need a lookahead block injected so the wayside fires
    # the reverse branch at the right moment:
    #
    # SW85 (host=85, rev=100): N-loop entry (100->85).
    #   Train on 99, next=100. Inject 100 as occupied so SW85 sees rev branch
    #   occupied and flips reverse.
    #
    # SW77 (host=77, rev=101): N-loop exit (77->101).
    #   Train arrives at 77 from 78 (second pass through N going -).
    #   Inject 101 as occupied when train is on 78 so SW77 flips reverse
    #   (101 in occupied + 78 not in occupied -> REVERSE fires).
    #
    # SW1 (host=1, rev=13): A-loop exit (1->13).
    #   Train arrives at 1 from 2 (A going -). Inject 13 as occupied when
    #   train is on 2 so SW1 flips reverse (1->13).
    #
    # SW57 (host=57, rev=0/yard): handled via authority_reach in wayside.
    # SW28 (host=28, rev=150): 150 is in build_switch_map reverse-merge edge.

    # Map: (current_block, next_block) -> block_to_inject
    _LOOKAHEAD_TRIGGERS = {
        (99,  100): 100,   # SW85: N-loop entry  (inject 100 when on 99)
        (79,  78):  101,   # SW77: N-loop exit   (inject 101 when on 79, fires when train reaches 78)
        (3,   2):   13,    # SW1:  A-loop exit   (inject 13  when on 3,  fires when train reaches 2)
    }

    for tid, tinfo in ext.items():
        if "Green" not in tinfo.get("line", ""):
            continue
        try:
            path_idx = int(tinfo.get("_path_idx", -1))
        except (TypeError, ValueError):
            continue
        if path_idx < 0 or path_idx >= len(_GREEN_OUTBOUND_PATH) - 1:
            continue
        cur_bn  = _GREEN_OUTBOUND_PATH[path_idx]
        next_bn = _GREEN_OUTBOUND_PATH[path_idx + 1]

        inject_bn = _LOOKAHEAD_TRIGGERS.get((cur_bn, next_bn))
        if inject_bn is None:
            continue

        _, _, inj_spd = _GREEN_BLOCK_INFO.get(inject_bn, ("", 50.0, 30.0))
        auth_km = _authority_to_dest_km("Green", inject_bn, 0)
        existing = green_data.get(inject_bn)
        if existing is None:
            green_data[inject_bn] = {
                "occupied":  True,
                "cmd_speed": inj_spd,
                "authority": auth_km,
            }
...
def _block_to_station_pos(line: str, block: int):
    ...
    is "before", "at", or "after" relative to the station block.
    Returns (None, None) for switch-only blocks with no nearby station.

    Derived programmatically from the wayside's GREEN_SIGNAL_BLOCKS and
    GREEN_STATION_NAMES so it stays consistent with the wayside.
    """
    # Build the maps once and cache them
    if not hasattr(_block_to_station_pos, "_cache"):
        _block_to_station_pos._cache = {}
        try:
            from wayside_controller import (
                GREEN_SIGNAL_BLOCKS, GREEN_STATION_NAMES,
                RED_SIGNAL_BLOCKS,   RED_STATION_NAMES,
            )
        except Exception:
            GREEN_SIGNAL_BLOCKS = set()
            GREEN_STATION_NAMES = {}
            RED_SIGNAL_BLOCKS   = set()
            RED_STATION_NAMES   = {}

        def _build_map(signal_blocks, station_names):
            """For each signal block, find the corresponding station label.
            With the simplified signal layout, only the station block itself
            maps to a station light. Switch-only and bidir-entry signals
            don't display on the CTC station-light overlay.
            """
            mapping = {}
            for blk in signal_blocks:
                if blk in station_names:
                    mapping[blk] = (station_names[blk], "at")
            return mapping

        _block_to_station_pos._cache["Green"] = _build_map(
            GREEN_SIGNAL_BLOCKS, GREEN_STATION_NAMES)
        _block_to_station_pos._cache["Red"] = _build_map(
            RED_SIGNAL_BLOCKS, RED_STATION_NAMES)

    cache = _block_to_station_pos._cache.get(line, {})
