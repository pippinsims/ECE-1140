...
def _mw_build_ctc_block_state(self, trains: list) -> None:
    ...
    for t in trains:
        ...
        if line_short == "Green":
            data_src, target = GREEN_LINE_BLOCKS, green_data
        elif line_short == "Red":
            data_src, target = RED_LINE_BLOCKS, red_data
        else:
            continue

        blocks = data_src.get(t.get("section", ""), [])
        info   = next((b for b in blocks if b[0] == bn), None)
        target[bn] = {
            "occupied":  True,
            "cmd_speed": float(info[3]) if info else 0.0,
            "authority": float(info[1]) / 1000.0 if info else 0.0,
        }
