...
class TrainControllerApp:
                ...
                try:
                    ...
                    while b"\n" in buf:
                        ...
                        if "passengers" in msg:
                            try:
                                t.passengers = int(msg["passengers"])
                            except Exception:
                                pass
    ...
    def _toggle_left_door(self):
        t = self.T
        try:
            moving = float(getattr(t, "current_speed", 0.0)) > 0.1
        except Exception:
            moving = True
        if moving:
            self._set_status("SAFETY: Cannot open doors while moving", C["fault"])
            return
    ...
    def _sync_doors(self):
        ...
        if   ld and rd: desired = 3
        elif rd:        desired = 1
        elif ld:        desired = 2
        else:           desired = 0
        # Safety: use backend guard if available (doors locked while moving).
        if hasattr(t, "set_doors") and callable(getattr(t, "set_doors")):
            try:
                t.set_doors(desired)
            except Exception:
                t.doors_state = desired
        else:
            t.doors_state = desired
