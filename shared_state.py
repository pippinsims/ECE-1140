...
class SharedState:
    ...
    def __init__(self):
        ...
        # Track Model -> Wayside: block occupancy (separate from CTC channel)
        # The CTC pushes block_data with all blocks marked occupied=False (it
        # doesn't know real occupancy). The Track Model is the authoritative
        # source for which blocks are actually occupied by trains.
        # {line: {block_num: bool}}
        self._track_occupancy: Dict[str, Dict[int, bool]] = {"Green": {}, "Red": {}}
        self._track_occ_version: int = 0
        self._track_occ_seen_by_wayside: int = -1

    # =========================================================================
    # Track Model -> Wayside: block occupancy
    # =========================================================================

    def push_track_occupancy(self, line: str, occupancy: Dict[int, bool]) -> None:
        """Track Model pushes per-block occupancy (the authoritative source).
        occupancy: {block_num: True/False}
        """
        with self._lock:
            self._track_occupancy[line] = dict(occupancy)
            self._track_occ_version += 1

    def poll_track_occupancy(self) -> Optional[Dict[str, Dict[int, bool]]]:
        """Wayside polls every 100ms. Returns {line: {blk: bool}} if new, else None."""
        with self._lock:
            if self._track_occ_version == self._track_occ_seen_by_wayside:
                return None
            self._track_occ_seen_by_wayside = self._track_occ_version
            return {line: dict(data) for line, data in self._track_occupancy.items()}

    def get_track_occupancy(self, line: str) -> Dict[int, bool]:
        """Latest track occupancy for ``line`` (copy). Does not affect polling."""
        with self._lock:
            return dict(self._track_occupancy.get(line, {}))
